//! the tree structures representing Tindalwic data.
...
/// returned by [UTF8::lines()] is the most efficient way to strip the indentation
...
impl<'a> UTF8<'a> {
    pub(crate) fn of(slice: &'a str, dedent: usize) -> Self {
        UTF8 { slice, dedent }
    }
    ...
    pub(crate) fn shortcut(&self, indent: usize) -> Option<&'a str> {
        if indent == self.dedent || self.one_liner() {
            Some(self.slice)
        } else {
            None
        }
...
/// # #[cfg(feature="alloc")]
/// # {
/// let comment = tindalwic::tree::Comment::wrap("with ~strikethrough~ extension");
///
...
impl<'a> Comment<'a> {
    ...
    pub(crate) fn is_funny(&self) -> bool {
        self.utf8.slice.starts_with('!') || self.utf8.slice.starts_with('\n')
    }
    /// Returned iterator produces a sub-slice for each line, stripped of indentation
    /// and line separators, in order, from `self`. Always produces at least one line.
    pub fn lines(&self) -> impl Iterator<Item = &'a str> {
...
impl<'a> Text<'a> {
    ...
    pub(crate) fn one_liner_in_list(&self) -> Option<&'a str> {
        if !self.utf8.one_liner() {
            ...
            Some(self.utf8.slice)
        }
    }
    pub(crate) fn one_liner_in_dict(&self, key: Key<'_>) -> Option<&'a str> {
        if !self.utf8.one_liner() {
            None
